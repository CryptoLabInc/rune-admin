...
func TestEngineSmoke(t *testing.T) {
	...
	// Search refuses an empty scope (fail-closed default-deny), so pass the
	// public-only sentinel (server.publicOnlyScopeSentinel): the smoke item is
	// inserted untagged (public), and untagged items pass any scope filter.
	hits, err := eng.Search(ctx, vec, 5, "__rune_no_access__")
	if err != nil {
