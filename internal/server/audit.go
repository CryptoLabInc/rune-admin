...
// are pinned by the golden compat tests.
type AuditEntry struct {
	...
	// Target names the entity an admin mutation acted ON (e.g. the user whose
	// token was issued/rotated/revoked), as opposed to UserID — who acted.
	// Empty for dataplane entries and for admin ops predating the field.
...
func NewAuditLogger(cfg AuditConfig) (*AuditLogger, error) {
	...
	if mode.File {
		...
		// Lumberjack handles daily-ish rotation by size + age. Retention is
		// 30 days; the size cap is high enough that audit volume drives
		// rotation by age, not size.
...
// ExtractSourceIP normalizes a gRPC peer address, which arrives in
// "ipv4:H:P", "ipv6:[::1]:P", or "unix:/path" form.
