...
class TestDecryptScores:
    ...
    def test_decrypt_valid_scores_flat(self, monkeypatch):
        ...
        mock_cipher.decrypt_score.return_value = _mock_decrypt_score_flat(scores)
    ...
    def test_decrypt_valid_scores_ivf(self, monkeypatch):
        ...
        mock_cipher = MagicMock()
        mock_cipher.decrypt_score.return_value = _mock_decrypt_score_ivf(
            [shard0, shard1], [5, 12]
        )
        monkeypatch.setattr('vault_mcp.cipher', mock_cipher)
        monkeypatch.setattr('vault_mcp.sec_key_path', '/fake/SecKey.json')
    ...
    def test_top_k_returns_correct_count(self, monkeypatch):
        ...
        for k in [1, 2, 3, 5]:
            result = decrypt_scores("DEMO-TOKEN-GET-YOUR-OWN-AT-ENVECTOR-IO", blob, top_k=k)
            data = json.loads(result)
            assert isinstance(data, list)
    ...
    def test_top_k_returns_highest_scores(self, monkeypatch):
        ...
        result = decrypt_scores("DEMO-TOKEN-GET-YOUR-OWN-AT-ENVECTOR-IO", blob, top_k=2)
        data = json.loads(result)
    ...
    def test_top_k_limit_enforced(self):
        ...
        result = decrypt_scores("DEMO-TOKEN-GET-YOUR-OWN-AT-ENVECTOR-IO", blob, top_k=15)
        data = json.loads(result)
    ...
    def test_malformed_blob_returns_error(self, monkeypatch):
    ...
    def test_empty_blob_returns_error(self, monkeypatch):
    ...
    def test_default_top_k_is_5(self, monkeypatch):
        ...
        result = decrypt_scores("DEMO-TOKEN-GET-YOUR-OWN-AT-ENVECTOR-IO", blob)
        data = json.loads(result)
    ...
    def test_ivf_topk_cross_shard(self, monkeypatch):
        ...
        result = decrypt_scores("DEMO-TOKEN-GET-YOUR-OWN-AT-ENVECTOR-IO", blob, top_k=4)
        data = json.loads(result)
