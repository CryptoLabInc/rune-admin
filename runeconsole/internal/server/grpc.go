...
func NewConsole(cfg *Config, tokenStore *tokens.Store, engine *crypto.Engine, audit *AuditLogger) *Console {
	return &Console{
		cfg:    cfg,
		tokens: tokenStore,
		engine: engine,
...
// caller's derived agent_dek leave the console here — by design: capture-side
...
func (v *Console) buildBundle(ctx context.Context, token string) (map[string]any, error) {
	...
	bundle := map[string]any{
		...
		// Capability flag: this console expects client-encrypted inserts.
...
func (s *ConsoleGRPC) openMeta(stored string) string {
	...
	if !utf8.Valid(pt) {
		slog.Warn("console: sealed metadata decrypted to non-UTF-8 bytes — returning envelope unopened (different team_secret or corrupted record)",
