...
type KeysParams struct {
	...
	// E.g., "/opt/runeconsole/runeconsole-keys" with KeyID "runeconsole-key" reads from
	// "/opt/runeconsole/runeconsole-keys/runeconsole-key/{Enc,Sec,Eval}Key(.json|.bin)".
	Root  string
	KeyID string
	Dim   int
}
...
// sealed (agent_dek), verbatim. The console performs no crypto here — it is a
...
// can only encrypt; decryption needs SecKey, which never leaves the console.
const (
