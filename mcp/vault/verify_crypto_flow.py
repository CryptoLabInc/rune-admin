...
def verify_flow():
    ...
    try:
        ...
        # Check similarity
        # Note: CKKS is approximate.
        # Structure might be diff (decrypted_scores might be list of floats)
        
        dec_arr = np.array(decrypted_scores)
        diff = np.abs(mock_scores - dec_arr[:len(mock_scores)]) # dec might be padded to dim?
        print(f"Max Diff: {np.max(diff)}")
        
        if np.max(diff) < 1e-4:
            print("SUCCESS: Crypto compatibility verified.")
        else:
            print("FAILURE: Decrypted values do not match.")
